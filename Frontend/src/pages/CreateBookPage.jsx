...
const CreateBook = () => {
  ...
  const handleIsbnChange = (e) => {
    ...
    if (value === '' || /^\d+$/.test(value)) {
      ...
      setIsbnError('ISBN must contain only numbers');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5005/books/admin', formData, {
        headers: {
      ...
      setMessage(error.response?.data?.message || 'Error creating book. Please try again.');
    }
  };
